...
public record Talk(String name, int length) {
    ...
    static Pattern pattern = Pattern.compile(String.format("\\d+|%s", LIGHTNING_TALK));

    static Talk fromString(String talk) {
        var matcher = pattern.matcher(talk);
        if (matcher.find()) {
            var talkLength = matcher.group();
            var length = talkLength.equals(LIGHTNING_TALK) ? LIGHTNING_TALK_LENGTH : Integer.parseInt(talkLength);
